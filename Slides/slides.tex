...
\end{verbatim}\end{tiny}
\end{example}
\column{.67\textwidth} % Left column and width
\begin{example}[tagged Wikicorpus]
...
%\begin{scriptsize}
%\begin{enumerate}
%\item Ignorar metadatos
%\item Separar palabra,lema y etiqueta
%\item Si la etiqueta empieza por FP $\Rightarrow$ tomar los dos primeros caracteres
%\item Si empieza por V tomar los 3 primeros
%\item Si empieza por NC tomar los 2 primeros + $3^{\circ}$ caracter
%\item En otro caso: tomar los dos primeros caracteres
%\item Separar palabras con guión bajo
%\end{enumerate}
%\end{scriptsize}
%\end{normalsize}
\end{columns}
...
\text{Tomar 100.000 palabras que más aparecen con la etiqueta que más aparecen}
...
\end{enumerate}
\end{scriptsize}
\begin{example}[reglas contextuales]
\begin{scriptsize}
\begin{verbatim}
VBD VB PREVTAG IN
...
%\end{enumerate}
%\end{scriptsize}
%\column{.93\textwidth} % Left column and width
%\begin{algorithm}[H]
%\begin{tiny}\fontsize{7.3}{8}\selectfont
 %   \begin{algorithmic}[1]
    %\algrenewcommand\alglinenumber[1]{\tiny #1:}
...
%: \begin{itemize}\fontsize{8}{7}\selectfont
...
\end{enumerate}
\begin{example}[encontrar posibles secuencias de tokens]
\begin{scriptsize}
...
\begin{scriptsize}
	...
	\If{$\text{token endsWith -es and token without 2 last chars endsWith (br,i,j,t,zn)}$} \State $\text{\Return word without its last char}$ \EndIf
	  ...
	  \If{$\text{token endsWith -es}$} \State $\text{\Return token without 2 last char}$ \EndIf
...
\begin{scriptsize}\fontsize{6.5}{1}\selectfont
	...
	\If{$\text{token is present singular $3^{\circ}$ person ending with -e }$} 
	...
	\If{$\text{token ends with (en,es)}$}  $\text{similar to last two teps}$\EndIf
...
\vspace{\baselineskip}
\large{\centerline{github \faicon{github} : CristinaHG/Spanish-NLP}}
\end{frame}
